...
const SearchForm = ({ searchMovies, searchFilter, setSearchFilter, page }) => {
  const [search, setSearch] = React.useState('');
  const [errorText, setErrorText] = React.useState('Фильмы');

  const handleSearch = (e) => { // отслеживать изменения в поле ввода
    setSearch(e.target.value);
  }

  const handleFilterClick = () => { // переключить фильтр поиска
    if (page === 'movies') {
  ...
  const handleSearchSubmit = (e) => { // найти фильмы
    e.preventDefault();
    if (search.length > 0) {
      if (page === 'movies') {
